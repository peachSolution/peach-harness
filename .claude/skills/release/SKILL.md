...
**버전 판단 기준** — 핵심: **"사용자 관점의 능력 변화 + 호환성"**으로 구분한다.

변경된 파일의 중요도나 변경량만으로 minor를 선택하지 않는다.
사용자 관점에서 새로 할 수 있는 핵심 개발 일이 생겼는지, 아니면 기존 기능이 더 정확하고 안정적으로 보강됐는지 먼저 판단한다.
...
| 기존 스킬 개선, references 추가/재구성, 워크플로우 세부 보완 | **patch** | OS별 분기 추가, 경계선 강화, 가이드 문서 추가 |
| 기존 핵심 스킬의 체크리스트, QA 기준, 상태 기록, 완료 보고 보강 | **patch** | TEST_ID 상태 기록 보강, Contract Gate 보고 항목 보강 |
| 릴리스, 시크릿, 검증 같은 운영 안정성 개선 | **patch** | pre-commit 훅 보강, 릴리스 판단 기준 명확화 |
| 보조/유틸 스킬 추가 | **patch** | 다이어그램 생성, 문서 변환, 도움말, 조회 보조 스킬 추가 |
| 새 핵심 스킬 또는 새 표준 개발 경로 추가 | **minor** | 새 핵심 `gen-*` 스킬, 새 `team-*` 스킬, 표준 플로우 단계 추가 |
| 기존 핵심 스킬의 사용자-facing 실행 능력 확장 | **minor** | 새 실행 모드, 새 입력/출력 계약, 새 에이전트 역할 또는 협업 구조 추가 |
| 하위호환 파괴, 구조 변경 | **major** | 스킬 이름/인자/산출물 경로 변경, alias 없는 rename, 기존 스킬 제거, 배포 구조 변경 |

핵심 스킬 파일이 수정됐다는 사실만으로 minor를 선택하지 않는다.
기존 핵심 스킬의 설명, 체크리스트, QA 기준, references, 상태 기록, 완료 보고가 보강된 경우는 patch다.
minor는 사용자가 새 핵심 개발 워크플로우, 새 실행 모드, 새 에이전트 협업 능력을 사용할 수 있게 된 경우에만 선택한다.
...
- 버전 타입(major/minor/patch)은 **사용자 관점의 능력 변화 + 호환성 기준으로 AI가 분석하여 결정**한다.
- 기존 스킬을 더 명확하고 안정적으로 만드는 개선은 patch다.
- 신규 스킬 추가라도 보조/유틸 성격이면 patch를 우선한다. minor는 새 핵심 개발 워크플로우, 새 실행 모드, 새 에이전트 협업 능력이 확장된 경우에만 사용한다.
