...
- `qmd`가 설치되어 있으면 파일 열기 전에 `qmd --index peach-harness query "키워드" -c peach-harness`로 위치를 먼저 찾는다.
- 컬렉션: `peach-harness`, URI 접두사: `qmd://peach-harness/`
- wiki 통합 검색: `qmd --index peach-harness query "키워드" -c peach-harness` (wiki + 소스 동시 검색)
- qmd가 느리거나 Metal 오류가 나면 `qmd --index peach-harness query "키워드" -c peach-harness --no-rerank` → `qmd --index peach-harness search "키워드" -c peach-harness` → `rg "키워드"` 순서로 대체한다.
...
# Apple Metal 오류 또는 리랭킹 지연 시
qmd --index peach-harness query "키워드" -c peach-harness --no-rerank
QMD_LLAMA_GPU=off qmd --index peach-harness query "키워드" -c peach-harness --no-rerank

# 인덱스 갱신
qmd --index peach-harness update && qmd --index peach-harness embed

# Apple Metal 오류 시 CPU 경로로 임베딩
QMD_LLAMA_GPU=off qmd --index peach-harness embed
```
...
- 스킬 본문에서 하네스 저장소의 `docs/` 문서를 직접 참조하지 않는다. 스킬은 사용자에게 배포될 때 `docs/`가 함께 배포되지 않으므로, 필요한 정책은 SKILL.md 본문 또는 스킬 내부 `references/`에 로컬 사본으로 포함한다.
- 단, 대상 프로젝트에서 생성·조회하는 산출물 경로 안내는 허용한다. 예: `docs/spec/`, `docs/qa/`, `docs/e2e-suite/`, `docs/기능별설명/`, `docs/wiki/`, `docs/erd/`, `docs/diagrams/`, `docs/스킬피드백/`, `docs/prd/`.
...
버전 판단은 변경된 파일의 중요도가 아니라 **사용자 관점의 능력 변화**로 결정한다.
기존 스킬을 더 명확하고 안정적으로 만드는 개선은 patch다.
minor는 새 핵심 개발 능력, 새 표준 경로, 새 실행 모드, 새 에이전트 협업 능력이 생긴 경우에만 선택한다.
애매하면 patch를 선택한다.
...
| 변경 유형 | 버전 | 예시 |
|----------|------|------|
| **patch** (x.x.+1) | 기존 기능을 더 정확하게, 안전하게, 잘 동작하게 만드는 변경 | SKILL.md 오류 수정, 참조 경로 수정, 기존 스킬 체크리스트/QA 기준/상태 기록 보강 |
| **patch** (x.x.+1) | 문서 수정, 오타, 가이드 보강, 운영 안정성 개선 | README/AGENTS/CHANGELOG 수정, 릴리스/시크릿/검증 기준 보강 |
| **patch** (x.x.+1) | 보조/유틸 스킬 추가 | 문서화, 분석, 변환, 조회, 보조 생성 스킬 추가 |
| **minor** (x.+1.0) | 사용자가 새로 선택할 수 있는 핵심 개발 경로 추가 | 새 핵심 `gen-*`/`team-*` 스킬 추가, 표준 플로우 단계 추가 |
| **minor** (x.+1.0) | 기존 핵심 스킬의 사용자-facing 실행 능력 확장 | 새 실행 모드, 새 입력/출력 계약, 새 에이전트 역할 또는 협업 구조 추가 |
| **major** (+1.0.0) | 하위호환 파괴, 구조 변경 | 스킬 이름/인자/산출물 경로 변경, alias 없는 rename, 기존 스킬 제거, 배포 구조 변경 |
...
→ **docs/04-워크플로우.md** 참조 (표준 5단계 직선형)
→ **docs/분석/2026-04-27-스킬재구성-분석.md** 참조 (과거 검증 기준 외부화, 통합 스킬 도입 의사결정 분석 기록)
