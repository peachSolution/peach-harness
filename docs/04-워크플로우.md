...
> 2026-05-03 갱신: intake 진입 게이트 + Claude/Codex 런타임 분기 반영
> 최신 실행 기준: 이 문서
> 상위 방향과 전략: `docs/01-하네스-방향과-전략.md`
...
기본값은 작은 작업 직접 수정이다. 요청이 작고 범위가 명확하면 `/peach-intake`를 거치지 않는다.

요청이 중대형이거나 PRD/Spec/proto/schema 준비 상태가 불분명하면 먼저 `/peach-intake`를 실행한다.

`peach-intake`는 구현하지 않는다. 작업 규모, Spec 필요 여부, DB 필요 여부, ui-proto 필요 여부, E2E 필요 여부만 판정하고 다음 스킬 경로를 제안한다.

```text
/peach-intake "요청 또는 PRD 경로"
→ SPEC_REQUIRED / DB_REQUIRED / PROTO_RECOMMENDED / READY_FOR_DEV 등으로 분류
→ 필요한 후속 스킬만 실행
```

바로 후속 스킬로 넘어가도 되는 경우:

- 단일 파일 버그처럼 요청과 범위가 명확하다.
- 이미 Spec/schema/proto가 준비되어 있다.
- 사용자가 실행할 스킬과 mode를 명확히 지정했다.

---

## 1. 표준 플로우 (필요시 Intake + 권장형 5단계)

```mermaid
flowchart TD
    Start([기능 개발 시작]) --> Intake["0. /peach-intake<br/>(중대형/모호할 때)"]
    Intake --> Spec["1. /peach-gen-spec<br/>(선택)"]
    ...
    class Intake,Spec,DB,Proto,Dev,E2E tier1
...
- **빡센 기획 후 자율 개발/검증**: 사람의 의도를 Spec/schema/ui-proto에 충분히 반영한 뒤 개발과 검증 루프는 AI가 최대한 자율 수행한다
- **Intake 우선**: 요청이 모호하면 구현 전에 `/peach-intake`로 경로를 판정한다
- **작은 작업 예외**: 단일 파일 버그, 명확한 문구 수정, 이미 범위가 고정된 변경은 직접 수정하거나 `peach-team-3a`로 처리한다
...
- **납품 흐름 연결**: `peach-team-dev`와 `peach-team-e2e`는 하나의 개발-검증 흐름으로 연결하되, 구현과 검증의 독립성은 유지한다
- **런타임 분기**: Claude Code는 team mode, Codex/skills.sh 일반 환경은 generic mode로 실행할 수 있다
...
TEST_ID 상태는 상세 추적용이다. 사용자 보고에서는 `미구현 / 구현중 / 구현완료 / 검증완료 / 차단`을 우선 쓰고, `Uxx/Ixx/Vxx` 코드는 근거가 필요한 상세 표에만 병기한다.
...
```
/peach-intake → 필요한 후속 스킬만 실행
```

적합 케이스:
- PRD, 회의 메모, 자연어 요청 중 무엇부터 해야 할지 애매함
- Spec/DB/ui-proto가 필요한지 판단이 필요함
- 작은 수정인지 팀 스킬이 필요한지 구분하고 싶음
...
| 상황 | 스킬 |
|------|------|
| 어떤 스킬로 시작할지 모름 | `/peach-intake` |
| PRD/요청 기반 작업 규모 판단 | `/peach-intake prd=<경로>` 또는 `/peach-intake "요청"` |
| Spec/DB/ui-proto 필요 여부 판단 | `/peach-intake` |
...
동작:
1. 작업 유형과 규모를 판정한다.
2. Spec/DB/ui-proto/E2E 필요 여부를 판단한다.
3. `READY_FOR_DEV`, `SPEC_REQUIRED`, `DB_REQUIRED`, `PROTO_RECOMMENDED`, `DECISION_REQUIRED` 같은 결과로 분류한다.
4. 후속 스킬 순서를 제안한다.
5. 코드, DB, proto, E2E 산출물은 직접 만들지 않는다.
...
런타임:
- Claude Code 팀 도구가 있으면 `references/runtime-adapter.md` 기준으로 team mode 실행
- Codex 또는 일반 skills.sh 환경에서는 같은 역할 큐를 generic mode로 실행
...
런타임:
- Claude Code 팀 도구가 있으면 team mode 실행
- Codex 또는 일반 skills.sh 환경에서는 generic mode로 시나리오 작성/QA/suite 실행을 순차 수행
...
| 날짜 | 변경 |
|------|------|
| 2026-05-03 | `peach-intake` 진입 게이트 추가. `peach-team-dev`/`peach-team-e2e`에 Claude team mode와 Codex/skills.sh generic mode 분기 반영 |
