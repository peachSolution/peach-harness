...
> **대상 스킬**: peach-wiki
> **작성 근거**: `~/source/{PROJECT}`에서 wiki ingest 후 다른 AI 세션이 `qmd update`와 `qmd embed`를 실행했고, Synology Drive 위의 `Obsidian/PARA`까지 함께 임베딩되면서 `fileproviderd`/`SynologyDriveFileProvider` CPU 상승이 관측됨
...
- 사용자는 `~/source/{PROJECT}`에서 작업 중이었지만, `para` 컬렉션도 전역 설정에 포함
- 결과적으로 Synology Drive의 `Obsidian/PARA`가 함께 스캔되었을 가능성이 높음

### 문제 #2: `{PROJECT}` 컬렉션 자체도 문서 전용이 아니라 소스까지 포함함

**원인**:
- 현재 `{PROJECT}` 컬렉션 패턴은 `**/*.{php,md,sql,js}` 이다.
- 따라서 `~/source/{PROJECT}` 기준으로 `md`뿐 아니라 `php/js/sql` 소스 파일도 함께 인덱싱/임베딩 대상이 된다.
...
**판단**:
- `~/source/{PROJECT}`에서 embed를 진행하면, 현재 설정상 해당 폴더 아래의 `md`, `php`, `sql`, `js`를 모두 처리하는 해석이 맞다.
- 단, plain `qmd embed`면 `{PROJECT}`만이 아니라 등록된 다른 컬렉션도 같이 돌 가능성이 있다.

### 문제 #3: Synology/CloudStorage 경로와의 충돌 위험이 문서화돼 있지 않음

**원인**:
- `para` 컬렉션이 `~/{CLOUD_STORAGE}/Obsidian/PARA`를 바라보는 상태에서,
...
- 현재: [skills/peach-wiki/SKILL.md](skills/peach-wiki/SKILL.md) 행 99
...
예:
```bash
QMD_COLLECTION="{PROJECT}"   # 현재 프로젝트명
...
- 현재: [skills/peach-wiki/SKILL.md](skills/peach-wiki/SKILL.md) 행 186
...
대상:
- [qmd-가이드.md](skills/peach-wiki/references/qmd-가이드.md)

추가할 내용:
- `~/.config/qmd/index.yml` 예시와 컬렉션별 path/pattern 해석
- `{PROJECT}` 패턴 `**/*.{php,md,sql,js}`처럼 소스까지 포함된다는 설명
...
**전제**:
- `~/.config/qmd/index.yml`에 `{PROJECT}`, `para` 등 복수 컬렉션이 등록돼 있어야 함
...
### 시나리오 2: `{PROJECT}` 컬렉션 범위 확인

**목적**: `~/source/{PROJECT}`에서 embed 시 어떤 파일이 대상이 되는지 명확히 검증
...
**기대 결과**:
- `{PROJECT}` 컬렉션의 `pattern: "**/*.{php,md,sql,js}"`가 확인된다.
...
1. 대상 스킬 현재 상태 확인
```bash
sed -n '1,260p' skills/peach-wiki/SKILL.md
```
