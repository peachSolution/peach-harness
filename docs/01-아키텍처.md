...
피치 하네스는 **Claude Code 플러그인과 skills.sh 배포를 함께 지원하는 AI 개발 운영 시스템**입니다.
...
├── peach-team-ui-proto/       ← Mock 기반 UI Proto 팀 조율
├── peach-team-dev/            ← 개발 팀 조율 (mode=backend/ui/fullstack)
├── peach-team-e2e/            ← E2E 통합 검증 조율
├── peach-team-3a/             ← 소규모 3-에이전트 조율
├── peach-doc-feature/         ← 기존 기능 as-is context pack 생성
...
`peach-intake`는 구현 스킬이 아니라 Layer 3 앞단의 진입 게이트다. PRD나 자연어 요청을 받아 Spec, DB, ui-proto, E2E 필요 여부를 판정하고 후속 스킬 경로만 제안한다.

### 런타임 어댑터

팀 스킬은 스킬별 독립 실행성을 유지하기 위해 `references/` 안에 실행 정책 사본을 포함한다.

| 런타임 | 실행 방식 |
|--------|-----------|
| Claude Code 플러그인 | `TeamCreate`, `TaskCreate`, `SendMessage`, worktree isolation 기반 team mode |
| Codex / 일반 skills.sh | 같은 역할 큐를 오케스트레이터가 순차 또는 제한 병렬로 수행하는 generic mode |

따라서 `peach-team-dev`, `peach-team-e2e`는 Claude 팀 기능이 없어도 절차를 해석할 수 있어야 한다. Claude 전용 도구명은 team mode에서만 강제하고, generic mode에서는 역할 큐와 체크리스트로 대체한다.
...
- **3단계 판정**: APPROVED / CONDITIONAL / REJECTED
  - APPROVED: 모든 검증 통과 → 완료
  - CONDITIONAL: 기계 검증 통과 + 구조적 의문 → 오케스트레이터가 수용/무시 판단
  - REJECTED: Must Follow 위반 또는 기계 검증 실패 → Ralph Loop
- **Dev 자기 검토**: QA 제출 전 3문항(범위/Must Follow/검증 통과) + 실행 증거 필수
- **Ralph Loop**: QA REJECTED 시 구조화된 피드백 주입 (단순 재시도 아님)
...
sequenceDiagram
    ...
    User->>Skill: /peach-team-dev
    Skill->>Dev: 코드 생성 지시
    Dev-->>Skill: 구현 완료 + 자기 검토 3/3

    loop Ralph Loop (최대 10회)
        Skill->>QA: 검증 요청 (worktree 격리)
        QA-->>Skill: 판정 (APPROVED/CONDITIONAL/REJECTED)
        alt APPROVED
            Skill->>Gate: /peach-qa-gate
            Gate-->>Skill: 증거 보고서 + 판정
            Skill-->>User: 완료 보고
        else CONDITIONAL
            Skill->>Skill: 오케스트레이터 판단 (수용/무시)
            alt 수용
                Skill->>Dev: 조건 수정 지시
                Dev-->>Skill: 수정 완료
            else 무시
                Skill->>Gate: /peach-qa-gate (근거 기록 후 완료)
            end
        else REJECTED
...
| 유형 | 스킬 | 검증 방법 |
|------|------|----------|
| **능력 향상형** (4) | gen-design, gen-spec, doc-feature, peach-help | 새 모델 시 A/B 테스트 |
| **선호도 인코딩형** (12) | gen-backend, gen-db, gen-store, gen-ui, team-ui-proto, add-api, add-cron, add-print, refactor-backend, refactor-frontend, team, team-refactor | Eval + 팀 루프 검증 |
