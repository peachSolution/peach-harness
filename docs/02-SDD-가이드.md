...
flowchart LR
    subgraph A["A. 신규 기능"]
        A1[기획서] --> A2[gen-spec] --> A3[gen-db] --> A4[agent-team]
    end
```
...
flowchart LR
    subgraph B["B. 기존 개선"]
        B1["gen-feature-docs<br/>(암묵지→명세서)"] --> B2["Context Pack<br/>선택적 주입"] --> B3[gen-spec] --> B4[구현]
    end
```

```mermaid
flowchart LR
    subgraph C["C. UI Proto 선행"]
        C1[gen-ui로 화면 이식] --> C2[gen-spec] --> C3[gen-db] --> C4["agent-team<br/>(mode=backend)"] --> C5["Store 연계<br/>(Mock→실제 API)"]
    end
```
...
| **B. 기존 개선** | gen-feature-docs로 암묵지를 명세서로 변환, 필요한 문서를 선택적으로 주입하여 Spec 확정 |
...
/peach-gen-db            # Spec 기반 DDL/마이그레이션 생성
    ↓
/peach-agent-team        # 팀 단위 코드 생성 (Backend → Store → UI)
```
...
기존 코드
    ↓
/peach-gen-feature-docs  # 코드 속 암묵지 → 명세서 변환 (Context Pack 4개 파일)
    ↓
개발자가 필요한 문서를 선택적으로 주입
    ↓
...
- `gen-feature-docs`는 **코드 속 암묵지를 명세서로 변환하는 단계**입니다. 기존 코드의 구조, 로직, 의존관계를 4개 문서(개요/로직/명세/TDD가이드)로 정리합니다.
- 개발자가 Context Pack 중 **필요한 문서를 판단하여 선택적으로** `gen-spec`의 컨텍스트로 주입합니다.
- AI가 주입된 문서를 기반으로 기존 구조를 이해한 상태에서 변경 Spec을 제안합니다.
- 소규모 수정은 `gen-feature-docs` 없이 바로 구현해도 됩니다.
...
/peach-gen-db            # Spec 기반 DDL/마이그레이션 생성
    ↓
/peach-agent-team mode=backend  # Backend + Store 생성
    ↓
