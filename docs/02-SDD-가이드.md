...
flowchart TD
    ...
    GENDB -->|"DDL/마이그레이션"| IMPL["peach-team 또는 직접 구현"]:::skill
...
/peach-gen-db            # Spec 기반 DDL/마이그레이션 생성
    ↓
/peach-team        # 팀 단위 코드 생성 (Backend → Store → UI)
```
...
/peach-gen-db            # Spec 기반 DDL/마이그레이션 생성
    ↓
/peach-team mode=backend  # Backend + Store 생성
    ↓
...
| **구현** | `peach-team` | 팀 단위 코드 생성 (Backend/Store/UI) |
...
```
AI Plan 모드 → (선택) /peach-gen-spec → /peach-gen-db → /peach-team
```
...
세션 2: Spec 로드 → AI Plan 모드 → /peach-gen-db → /peach-team
