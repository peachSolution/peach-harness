...
| **B. 기존 개선** | gen-feature-docs로 암묵지를 명세서로 변환, 폴더를 주입하면 AI가 개요 기반으로 자동 선택하여 Spec 확정 |
...
기존 코드
    ↓
/peach-gen-feature-docs  # 코드 속 암묵지 → 명세서 변환 (Context Pack 폴더)
    ↓
...
- `gen-feature-docs`는 **코드 속 암묵지를 명세서로 변환하는 단계**입니다. 기존 코드의 구조, 로직, 의존관계를 주제별 문서(개요가 인덱스 역할)로 정리합니다.
