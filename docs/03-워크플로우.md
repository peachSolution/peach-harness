...
| DB/ERD/schema | 구조 기준. `peach-team-dev`가 API/Store를 만들 때 참조하는 확정 기준 |
...
```text
PRD 전체 분석
→ /peach-gen-db (DB/ERD/schema 1차 초안)
...
| 2 | `peach-gen-db` | 마이그레이션 + `api/db/schema/...` → 본 개발의 DB 기준. PRD-first에서는 1차 초안 생성 가능 |
