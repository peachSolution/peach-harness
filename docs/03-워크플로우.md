...
flowchart TD
    ...
    DB --> Proto["3. /peach-gen-ui-proto<br/>(ui-proto 저장소)"]
    Proto --> Review{"기획자 검토"}
    Review -->|수정 필요| Proto
...
```
/peach-gen-ui-proto (단독 모드) → (필요시 사후) /peach-gen-spec → /peach-gen-db → 본 프로젝트 진행
```
...
| 2 | `peach-gen-db` | 마이그레이션 + `api/db/schema/...` → 본 개발의 DB 기준 |
| 3 | `peach-gen-ui-proto` | ui-proto 태스크 폴더 (`_spec.md` + Vue 화면) → 본 개발 검증 기준 |
