...
> 2026-04-27 갱신: ui-proto 단계 표준 흡수 + 통합 스킬 도입 (`peach-team-dev`, `peach-team-e2e`)
> 의사결정 근거: `docs/05-스킬재구성-2026-04-27.md`
...
flowchart TD
    ...
    Spec --> Proto["2. /peach-gen-ui-proto<br/>(ui-proto 저장소)"]
    Proto --> Review{"기획자 검토"}
    Review -->|수정 필요| Proto
    Review -->|승인| Dev["3. /peach-team-dev<br/>proto=&lt;경로&gt;"]
    Dev --> E2E["4. /peach-team-e2e<br/>proto=&lt;경로&gt;"]
    E2E --> End([완료])
    ...
    class Spec,Proto,Dev,E2E tier1
...
| 2 | `peach-gen-ui-proto` | ui-proto 태스크 폴더 (`_spec.md` + Vue 화면) → 본 개발 검증 기준 |
| 3 | `peach-team-dev` | 본 프로젝트 코드 + `docs/spec/` 사본 → E2E 검증 기준 |
| 4 | `peach-team-e2e` | E2E 시나리오 + 부합 검증 결과 → 릴리스. 실행 중 오류 발견 시 AI가 자율 보완 루프 수행 (시나리오 .js 수정, suite md 보정). 본 프로젝트 코드 미스매치는 `peach-team-dev`로 위임 |

순서를 어기면:
- **proto 없이 dev** → 화면 검증 기준 부재로 기획자 검토가 늦어짐
- **dev 없이 e2e** → 검증 대상 부재
...
상세: `docs/05-스킬재구성-2026-04-27.md` §2-2
...
| 신규 기능 — ui-proto 작성 | `/peach-gen-ui-proto` |
...
| 단계 | 스킬 |
|------|------|
| DB 스키마/마이그레이션 | `/peach-gen-db`, `/peach-db-migrate`, `/peach-db-extract-schema`, `/peach-erd` |
...
3. 충돌 시 사용자 확인
4. ui-proto 화면 폴더를 검증 기준 컨텍스트로 주입
...
| 날짜 | 변경 |
|------|------|
| 2026-04-27 | ui-proto 표준 흡수, `peach-team-dev`/`peach-team-e2e` 도입, 리팩토링 스킬 폐기 |
