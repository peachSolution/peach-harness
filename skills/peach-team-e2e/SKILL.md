...
분류 결과에 따라 (a)/(b)는 안전 조건을 만족하면 `peach-team-dev`로 자동 위임하고, (c)는 시나리오 자동수정을 수행한다. (d)는 `peach-gen-spec` 보강으로 넘기며, (e)는 사용자 확인 또는 기준 보강으로 분기한다. 반복 상한과 완료 보고 기준은 `references/qa-policy.md`를 따른다.
