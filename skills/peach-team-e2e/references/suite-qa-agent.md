...
description: |
  ...
  실패 시 미스매치를 6가지로 분류(Spec 위반 / proto 불일치 / 시나리오 오류 / suite 구성 오류 / PRD 해석 누락 / 기준 모호).
...
실패 또는 검증 미부합 시 `미스매치-분류.md`에 따라 6가지로 분류:
...
| (d) suite 구성 오류 | Step 순서, 데이터 전달, fixture, 검증 포인트 조합 오류 → suite md 수정 필요 보고 |
| (e) PRD 해석 누락 | PRD에는 있으나 Spec/ui-proto에 없음 → PRD_TO_SPEC_REQUIRED 보고 |
| (f) 기준 모호 | Spec/proto 근거 부족 또는 충돌 → 사용자 확인 필요로 보고 |

분류 모호 시 (f)로 보고하고 오케스트레이터 판단을 요청한다.
...
### (d) suite 구성 오류
- [Step N]: [증상] → [Step 순서/데이터 전달/fixture/검증 포인트/경로] 문제
...
- (c): 자동수정 가능 (X회 시도 결과: ...)
- (d): suite md 수정 필요, 실패 지점부터 재실행 권고
- (e): PRD_TO_SPEC_REQUIRED, Spec 보강 필요
- (f): 사용자 확인 또는 기준 보강 필요
```

## 자동수정 한계

(c) 분류만 시나리오 자동수정 시도. (d) 분류는 suite-qa가 직접 수정하지 않고 suite-dev 재실행을 요청한다. (a)/(b)는 suite-qa가 직접 수정하지 않고 오케스트레이터가 자동 위임 안전 조건을 판단한다.
...
- (d) 분류 시 suite md 직접 수정 금지. suite-dev 재실행에 필요한 근거만 보고
- worktree 격리 유지
- suite-dev 출력만 보고 판정

### May Adapt
- (c) 분류의 자동수정 시도 (최대 3회)
- (d) suite 구성 오류 의심 시 단위 시나리오 단독 통과 여부, Step 순서, 데이터 전달 근거 확인
