...
Apple Silicon에서 `qmd query`, `qmd embed`, `qmd status`가 아래 메시지 뒤에 멈추면 Metal 백엔드 컴파일 실패로 본다.
...
- `QMD_LLAMA_GPU=off`는 qmd가 지원하는 CPU 강제 옵션이다. 느리지만 검색과 임베딩을 완료할 수 있다.
