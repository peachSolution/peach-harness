...
ui-proto 화면을 만들고 검증하는 팀 오케스트레이터.
...
| 스킬 | 책임 |
|------|------|
| `peach-team-ui-proto` | Mock 기반 기획 검토용 화면 + `_spec.md` + 태스크 메타 생성/검증 |
...
ui-proto는 본 개발 코드가 아니다. 프로덕션 코드 수정, 실제 API 연동, E2E 최종 판정은 이 스킬에서 처리하지 않는다.
...
이 스킬은 입력에 따라 **자동으로 팀 모드/단독 모드**로 분기한다.
...
| **proto-ui-qa** | Spec 부합 검증 + 시각/UX 검증 (독립 worktree, 읽기전용) |
...
기획자가 직접 작업하는 경우. 자체 검증만 수행 (별도 QA 에이전트 없음).
