...
### 화면 목록
| 화면 | 경로 | 관련 TEST_ID | UI Proto 상태 | 목적 |
|------|------|--------------|----------------|------|
...
### Spec 미반영/검증 불가
| TEST_ID | UI Proto 상태 | 사유 | 후속 처리 |
|---------|----------------|------|-----------|
```

작성 규칙:

- Spec에 있는 `TEST_ID`가 화면에 없으면 숨기지 말고 `Spec 미반영/검증 불가`에 기록하고 `U01 미반영(NOT_MAPPED)` 또는 `U90 차단(BLOCKED)`로 남긴다.
- 화면/액션/완료/오류 상태에 반영된 TEST_ID는 `U03 반영완료(MAPPED)`로 남긴다.
- 일부 화면만 있거나 결과 상태가 부족하면 `U02 일부반영(PARTIAL)`로 남긴다.
- Mock 데이터 한계나 proto 범위 밖이라 화면으로 검증할 수 없는 항목은 `U80 검증불가(UNTESTABLE)`로 남긴다.
...
- 입력 Spec에 `TEST_ID별 상태` 표가 있으면 UI Proto 상태 축만 갱신하고, 구현 상태(`Ixx`)와 검증 상태(`Vxx`)는 변경하지 않는다.
- `TEST_ID별 상태` 표의 UI Proto 상태를 갱신하면 `상태 요약`의 전체 UI Proto 상태, UI 반영 완료 수, Blocked 수, 마지막 확인일, 잔여 리스크도 함께 재계산한다.
