...
- AI 친화적 구조 (토큰 최소화)
- DB 스키마 기본 설계 포함
- **1차 완성도 기준 생성**: `team-dev`와 `team-e2e`가 처음 실행에서 누락을 줄일 수 있도록 권한/상태/오류/외부의존/TEST_ID를 구조화
- test-data 가이드 코드 참조
- **FK(Foreign Key)는 절대 생성하지 않음** (참조 무결성은 애플리케이션에서 처리)
- AI가 먼저 분석하고 제안, 개발자가 확인/수정/지시
- 하네스는 강제 파이프라인이 아니라 개발자가 판단해 조합하는 도구 세트
- 컨텍스트 주입은 개발자가 판단
- PRD는 원천 자료, Spec은 구현 기준이다. PRD에만 있는 요구는 `PRD_TO_SPEC_REQUIRED`로 남기고 Spec에 반영된 뒤 구현 단계로 넘긴다
...
├── 메타 정보
├── 1. 기능 범위 (CRUD, 파일)
├── 2. TEST_ID 상태 추적 (UI Proto 반영, 구현, 검증 상태 분리)
├── 3. 1차 완성도 기준 (TEST_ID, 권한, 상태, 오류, 외부의존)
├── 4. UI/화면 흐름 기준 (UI Proto 없을 때의 최소 검증 기준 포함)
├── 5. DB 스키마 (후보 컬럼, 인덱스, 개발 중 변경 가능성)
├── 6. 파일 목록 (Backend, Frontend)
└── 7. 참조
```
...
### 시나리오 D: PRD-first 신규 개발
PRD 전체와 `peach-gen-db`의 DB/ERD/schema 1차 초안을 함께 주입.
AI가 PRD 요구를 TEST_ID, 권한, 상태, 오류, 화면 흐름, DB 변경 후보로 정제한다.
...
- 개발자 ID (예: pdj)
- PRD 원천 경로와 DB/ERD 초안 경로 (PRD-first인 경우)
...
UI Proto 없이 본 개발 또는 E2E를 진행할 수 있으므로, 선택한 UI 패턴과 별개로 다음을 반드시 수집한다.

- 화면 유형: 목록/상세/등록/수정/모달/탭/일괄처리 등
- 주요 액션: 검색, 저장, 수정, 삭제, 승인, 반려, 업로드, 다운로드 등
- 완료 상태: 액션 성공 후 보이는 화면, 토스트, 목록 반영, 상태값 변경
- 오류 상태: 입력 검증 실패, 권한 실패, 외부 연동 실패, 서버 오류 표시 방식
...
DB 컬럼은 1차 후보로 정의한다. 개발 중 필드 부족이나 타입 변경이 발견될 수 있으므로, Spec에는 다음을 함께 남긴다.

- 후보 컬럼: 현재 요구사항으로 확정 가능한 컬럼
- 변경 가능 컬럼: 화면/정책 확정 후 추가·수정될 가능성이 있는 컬럼
- 변경 트리거: 어떤 요구사항 또는 `TEST_ID`에서 DB 변경이 필요해지는지

### 4단계: 추가 요구사항 + 비즈니스 규칙
- 도메인 특화 규칙
- 권한 경계 (역할, 접근 범위, 필수 조건)
- 상태 전이 (상태값, 변경 가능 조건, 금지 조건)
- 오류 케이스 (입력 검증, 권한 실패, 외부 실패, 서버 오류)
- 외부 의존성 (외부 API, 파일, 엑셀, 배치, 알림 등)
- TDD 테스트 적용 여부
...
⑤ **TEST_ID 매핑**: 요구사항별 `T-001` 형식 ID 부여
⑥ **테스트 시나리오**: CRUD 실행기 + 비즈니스 test case + E2E 후보
⑦ **화면 흐름 요약**: UI Proto가 없을 때 team-dev/team-e2e가 사용할 화면·액션·상태 기준
⑧ **DB 변경 후보**: 개발 중 `DB_CHANGE_REQUIRED`로 전환될 수 있는 컬럼/인덱스/상태값 후보
⑨ **PRD 반영 추적**: PRD 요구가 Spec `TEST_ID`로 매핑됐는지 확인하고 누락은 `PRD_TO_SPEC_REQUIRED`로 분리
⑩ **TEST_ID 상태 추적 기준**: Spec 전체 상태는 TEST_ID별 UI Proto 반영/구현/검증 상태의 집계로 판단
...
- `SOURCE_PRD` → PRD 원천 경로 또는 요약
- `PRD_TO_SPEC_TRACE` → PRD 요구와 Spec TEST_ID 매핑
- `PRD_TO_SPEC_REQUIRED` → PRD에는 있으나 Spec 반영 전 결정/정제가 필요한 항목
- `SPEC_STATUS_SUMMARY` → TEST_ID 기반 전체 Spec/UI/구현/검증 상태 집계
- `TEST_ID_STATUS_MATRIX` → TEST_ID별 UI Proto 반영 상태, 구현 상태, 검증 상태, 근거, 잔여 이슈
- `FIRST_PASS_TEST_MATRIX` → 1차 완성도 검증 매트릭스
- `PERMISSION_MATRIX` → 권한 경계 매트릭스
- `STATE_TRANSITIONS` → 상태 전이 표
- `ERROR_CASES` → 오류 케이스 표
- `EXTERNAL_DEPENDENCIES` → 외부 의존성 표
- `TEST_COVERAGE_MAPPING` → TEST_ID별 TDD/E2E 매핑
- `SCREEN_FLOW_SUMMARY` → UI Proto 없는 경우의 화면 흐름 요약
- `DB_CHANGE_CANDIDATES` → 개발 중 변경 가능성이 있는 컬럼/인덱스/상태값 후보
- `DATA_FLOW_DIAGRAM` → CRUD 선택(Y)에 따라 동적 생성된 sequenceDiagram (아래 로직 참조)
- `DESIGN_MEMO` → 설계 결정 근거 및 기각된 대안 (6단계 수집)

#### TEST_ID 상태코드 생성 규칙

Spec 생성 시점에는 구현 전 상태가 기본값이다. 구현 완료 여부는 사람이 임의로 `완료`라고 쓰지 않고 TEST_ID별 상태를 집계해 판단한다. 상태값은 사람이 읽기 쉬운 한글 라벨과 후속 스킬이 집계하기 쉬운 코드/키를 함께 쓴다.
...
- `S04 구현완료(IMPLEMENTED)`와 `S05 검증완료(VERIFIED)`는 분리한다. `S04`는 코드가 있다는 뜻이고, `S05`는 증거가 있다는 뜻이다.
- `TEST_ID_STATUS_MATRIX`를 수정하면 `SPEC_STATUS_SUMMARY`도 같은 집계 규칙으로 즉시 재계산한다.

Spec 생성 시 기본 치환값:
- UI Proto나 이식된 화면이 없는 신규 Spec은 `SPEC_STATUS_SUMMARY`를 `전체 Spec 상태: S02 확정(APPROVED)`, `전체 UI Proto 상태: U01 미반영(NOT_MAPPED)`, `전체 구현 상태: I01 구현전(TODO)`, `전체 검증 상태: V01 검증전(UNVERIFIED)`, `총 TEST_ID`, `UI 반영 완료: 0`, `구현 완료: 0`, `검증 완료: 0`, `Blocked: 0`, `마지막 확인일: Spec 생성일`, `잔여 리스크: 구현 전` 행으로 생성한다.
- UI Proto나 이식된 화면이 없는 신규 Spec은 `TEST_ID_STATUS_MATRIX`의 모든 TEST_ID를 `UI Proto 상태: U01 미반영(NOT_MAPPED)`, `UI 근거: -`, `구현 상태: I01 구현전(TODO)`, `구현 근거: -`, `검증 상태: V01 검증전(UNVERIFIED)`, `검증 근거: -`, `잔여 이슈: 구현 전`으로 생성한다.
- 시나리오 C(UI Proto 선행) 또는 이미 이식된 화면을 입력으로 Spec을 생성하는 경우에는 화면/액션/완료/오류 상태 근거를 분석해 TEST_ID별 UI Proto 상태를 `U02/U03/U80/U90`로 산정한다. 이 경우 `U01 미반영(NOT_MAPPED)`을 일괄 적용하지 않는다.
- UI Proto 상태를 입력 근거로 산정한 뒤 `SPEC_STATUS_SUMMARY`의 전체 UI Proto 상태와 UI 반영 완료 수를 재계산한다.
- 후속 구현/검증 스킬 또는 작업자가 상태를 갱신할 때는 PR, 커밋, 테스트 명령, E2E 결과, `docs/qa/` 보고서 같은 근거를 함께 남긴다.
...
**현재 상태:**
- Spec 상태: S02 확정(APPROVED)
- UI Proto 상태: U01 미반영(NOT_MAPPED)
- 구현 상태: I01 구현전(TODO)
- 검증 상태: V01 검증전(UNVERIFIED)
- 기준: TEST_ID별 상태를 집계해 전체 상태를 판단
