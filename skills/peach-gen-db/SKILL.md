---
name: peach-gen-db
description: DB DDL/마이그레이션 생성 전문가. "테이블 만들어줘", "DB 스키마 생성", "마이그레이션 생성" 키워드로 트리거. PRD 문서 입력 시 DB/ERD 후보를 먼저 만들고, 확정 Spec 또는 명확한 테이블 구조에서는 dbmate 마이그레이션 파일을 생성.
---
...
PRD를 입력받은 경우 PRD는 원천 자료로만 사용한다. 이 스킬은 PRD 전체에서 엔티티, 관계, 상태값, 이력/로그 필요성을 추출해 DB/ERD 후보 1차 초안을 만든다.

PRD만으로 확정할 수 없는 항목은 임의로 확정하지 않고 `DB_DECISION_REQUIRED`로 분리한다. PRD-only 상태에서는 실제 마이그레이션 생성을 강행하지 않는다. 결정 보류가 없고 사용자 확인 또는 Spec 정제를 거쳐 구조가 확정된 뒤 schema/migration을 생성한다.
...
- 입력이 확정 Spec이다.
- PRD-first 초안에 `DB_DECISION_REQUIRED`가 없고, 사용자가 schema/migration 생성을 승인했다.
- 기존 schema에 대한 명확한 `DB_CHANGE_REQUIRED` 요청이다.
...
4. **결정 필요 항목 분리**: 확정 불가한 DB 판단은 `DB_DECISION_REQUIRED`로 보고하고 migration 생성 중단
...
10. **마이그레이션 파일 생성**: migration 생성 조건을 만족할 때만 `api/db/migrations/[timestamp]_create_[테이블명]_table.sql` 생성
