...
- `api/src/modules/[모듈명]/` → `front/src/modules/[모듈명]/`
- `api/src/modules-domain/[모듈명]/` → `front/src/modules-domain/[모듈명]/`
- `api/src/modules-external/[모듈명]/` → `front/src/modules-domain/[모듈명]/` (프론트는 domain으로 통합)

`_common/constants/` 존재 시 프론트에서도 상수 활용을 고려합니다.
...
4. **구조 제안 (May Suggest)**:
   ...
   - Store 파일 분리 (예: 조회 전용 Store + 관리 전용 Store)
   - 파생 상태(computed) 구조 제안
...
> 모듈 생성 경로는 1단계에서 감지된 Backend 대응 경로를 따릅니다. 기본값: `front/src/modules/[모듈명]/`
