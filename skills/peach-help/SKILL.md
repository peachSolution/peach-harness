...
| 유형 | 예시 질문 | 참조 |
|------|----------|------|
| **워크플로우** | "새 모듈 만들려면?", "순서가 어떻게 돼?" | 하네스 프로젝트의 워크플로우 문서 |
| **스킬 추천** | "어떤 스킬 써야 해?", "이 상황에 맞는 스킬은?" | 아래 스킬 요약표 |
| **시스템 이해** | "Ralph Loop이 뭐야?", "에이전트 팀이란?", "AGENTS.md가 뭐야?" | 하네스 프로젝트의 아키텍처 문서 |
| **유지보수** | "기존 기능 수정하려면?", "리팩토링 어떻게 해?" | 워크플로우 문서 + 대상 프로젝트 기능별 설명 확인 |
...
- **워크플로우 유형** → 하네스 프로젝트의 워크플로우 문서 읽기
- **시스템 이해 유형** → 하네스 프로젝트의 아키텍처 문서 읽기
- **스킬 추천 유형** → 아래 스킬 요약표로 답변 (문서 로드 불필요)
- **유지보수 유형** → 워크플로우 문서 읽기 + 대상 프로젝트의 기능별 설명 존재 여부 확인
