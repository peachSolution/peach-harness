...
### Added
- `peach-team-dev`에 대규모 작업용 기능 큐 워크플로우와 상태 파일 템플릿 추가

### Changed
- `peach-team-dev`의 기능별 상태 파일, Contract Gate 근거, E2E 인계 기록 기준 보강
- `peach-team-e2e`의 미스매치 분류를 6가지로 확장하고 suite 구성 오류 처리 경로 추가
- 사용자 보고용 상태 표현을 `Uxx/Ixx/Vxx` 코드보다 쉬운 한글 상태 중심으로 정리
- README와 워크플로우 문서를 Tier 0/1 중심 진입점과 Tier 2 전문가용 색인 구조로 정리
- 팀 스킬 개선 방향을 새 게이트 추가보다 진입점 축소, 큐/상태 인계 표준화, 사용자-facing 상태 표현 단순화 중심으로 정리
...
- 스킬 본문에서 하네스 저장소 `docs/` 문서 직접 참조를 제거하고, 배포 가능한 SKILL.md 본문/스킬 내부 references 기준으로 정리
- `peach-team-ui-proto`, `peach-team-dev`, `peach-team-e2e`의 역할 경계와 TEST_ID 상태 갱신 기준 보강
- 릴리스 Semver 기준을 사용자 관점의 능력 변화 기준으로 명확화하고, 기존 핵심 스킬 보강은 patch로 판단하도록 정리

### Fixed
- YouTube 트랜스크립트 캐시 디렉터리를 git 추적 대상에서 제외
